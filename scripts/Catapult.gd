...
func _on_GameDir_pressed() -> void:
	...
	if DirAccess.new().dir_exists(gamedir):
...
func _on_UserDir_pressed() -> void:
	...
	if DirAccess.new().dir_exists(userdir):
...
func _on_InstallsList_item_activated(index: int) -> void:
	...
	if DirAccess.new().dir_exists(path):
		OS.shell_open(path)
...
#				_lst_installs.set_item_custom_fg_color(curr_idx, Color(0, 0.8, 0))
	...
	if game in _installs:
		...
		_btn_resume.disabled = not (DirAccess.new().file_exists(Paths.config.plus_file("lastworld.json")))
